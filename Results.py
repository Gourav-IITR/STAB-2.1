...
def result_fn(root,possibleSolutions,corData,numSieves,numStockPiles,entries,sieve_entries,mainroot,wp,pan):
    ...
    if(numsol>0):
        ...
        canvas.create_text(
            ...
            173.0,
            anchor="nw",
            text="Find solutions that are in +/-",
            fill="#273340",
            font=("OpenSansRoman SemiBold", 20 * -1)
        )

        stretchingError = IntVar()
        stretchingError.set(1)
        newError=ttk.Combobox(window, textvariable=stretchingError,width=5,height=24,state="readonly")
        newError['values']=(1,2,3,4,5)
        newError.place(x=743,y=175, anchor="nw")
        newError.current()
        ...
        button_4 = Button(
            ...
            borderwidth=0,
            highlightthickness=0,
            command=lambda: calculate2(numStockPiles,numSieves,entries,sieve_entries,window,mainroot,wp,pan,stretchingError),
            relief="flat"
        )
        button_4.place(
            x=927.0,
