...
def calculate(numStockPiles,numSieves,entries,sieve_entries,root,mainroot,wp,pan,StretchingError):
    flag=True
    ...
    with open(filename) as f:
        ...
        while(contents):
            ...
            for i in range(numSieves):
                if (corData[i][1]+StretchingError)<=100:
                    x=corData[i][1]+StretchingError
                else:
                    x=100
                if (corData[i][0]-StretchingError)>0:
                    y=corData[i][0]-StretchingError
                else:
                    y=0
    ...
    result_fn(root, possibleSolutions, corData, numSieves, numStockPiles,entries,sieve_entries,mainroot,wp,pan)
